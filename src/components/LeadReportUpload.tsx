...
export function LeadReportUpload({onImported,reviewUnresolved=false}:{onImported?:()=>void|Promise<void>;reviewUnresolved?:boolean}){
  ...
  const excludable=reviewRows.filter(row=>!row.excluded);
  const bulkEligible=rows.filter(row=>row.selectedFixtureId&&!row.imported);
  const allEligibleIncluded=bulkEligible.length>0&&bulkEligible.every(row=>row.confirmed&&!row.excluded);
  const allRowsExcluded=reviewRows.length>0&&reviewRows.every(row=>row.excluded);
  ...
  async function excludeAllRows(){
    ...
    setBusy(true);
    try{
      const excludedIds=new Set(excludable.map(row=>row.id));
      const results=await Promise.all(excludable.map(row=>db.from('lead_testing_report_rows').update({confirmed_fixture_id:null,user_confirmed:false,match_status:'excluded'}).eq('id',row.id)));
      const failed=results.find(result=>result.error);
      if(failed?.error)throw failed.error;
      setRows(current=>current.map(row=>excludedIds.has(row.id)?{...row,confirmed:false,excluded:true}:row));
      toast.success(`${excludable.length} result${excludable.length===1?'':'s'} excluded.`);
  ...
  return <div className="space-y-3"><div className="card-soft flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold">Review · {fileName}</p><p className="text-xs text-muted-foreground">{ready} included and ready · {needsReview} need review</p></div>{reviewRows.length>0&&<div className="flex flex-wrap items-center gap-2"><label className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-sm font-semibold ${allEligibleIncluded?'border-primary/40 bg-primary/5 text-primary':'border-border bg-background'} ${busy||bulkEligible.length===0?'cursor-not-allowed opacity-60':'cursor-pointer'}`}><Checkbox checked={allEligibleIncluded} disabled={busy||bulkEligible.length===0} onCheckedChange={checked=>void(checked===true?includeAllRows():clearAllRows())}/><span>Include All ({bulkEligible.length})</span></label><label className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-sm font-semibold ${allRowsExcluded?'border-destructive/40 bg-destructive/5 text-destructive':'border-border bg-background'} ${busy?'cursor-not-allowed opacity-60':'cursor-pointer'}`}><Checkbox checked={allRowsExcluded} disabled={busy} onCheckedChange={checked=>void(checked===true?excludeAllRows():clearAllRows())}/><span>Exclude All ({reviewRows.length})</span></label></div>}</div>{rows.map(row=><ReviewCard key={row.id} row={row} fixtures={fixtures} onCreate={()=>createFixtureForRow(row)} onChange={(patch,rematch)=>void changeRow(row,patch,rematch)}/>)}
