...
import { Search, Droplets, AlertTriangle, Download, Upload, Wrench, ArrowRight, PlusCircle } from 'lucide-react';
...
export default function Dashboard() {
  ...
  return (
    <div className="px-4 pt-6 pb-8">
      ...
      <Link
        to="/add"
        className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-accent py-3.5 text-sm font-semibold text-accent-foreground shadow-sm"
      >
          ...
          {results.length === 0 ? (
            ...
            results.map((f) => (
              <Link key={f.id} to={`/fixture/${f.id}`} className="flex items-center justify-between border-b py-2 last:border-0">
                <div>
                  <p className="text-sm font-medium text-foreground">{f.buildingName} — Fl {f.floor}, {f.roomNumber}</p>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <div className="card-soft p-4">
          ...
          <p className="text-xs text-muted-foreground">Fixtures in view</p>
        </div>
        <div className="card-soft p-4">
      ...
      <div className="mt-4 card-soft p-4">
        ...
        {progressRows.length === 0 ? (
          <p className="mt-3 text-sm text-muted-foreground">No buildings yet — add a campus and building to begin.</p>
        ) : (
              ...
              return (
                <div key={building.id} className="rounded-2xl bg-secondary/30 p-3">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2 text-[10px] font-semibold">
                    <span className="rounded-full bg-status-good/15 px-2 py-0.5 text-status-good">{FLOOR_STATUS_LABELS.Done}</span>
                    <span className="rounded-full bg-status-warning/15 px-2 py-0.5 text-status-warning">{FLOOR_STATUS_LABELS.InProgress}</span>
                    <span className="rounded-full bg-secondary px-2 py-0.5 text-secondary-foreground">{FLOOR_STATUS_LABELS.NotStarted}</span>
                    {counts.Restricted > 0 && (
                      <span className="rounded-full bg-status-urgent/15 px-2 py-0.5 text-status-urgent">{FLOOR_STATUS_LABELS.Restricted}</span>
                    )}
                  </div>
                </div>
              );
