...
export default function LeadTestingResults(){
  ...
  useEffect(()=>{void(async()=>{const db=supabase as any;const result=await db.from('lead_testing_report_rows').select('id,proposed_fixture_id,confirmed_fixture_id,imported_testing_round_id,match_status,user_confirmed,lead_testing_report_uploads(district_or_organization)').is('imported_testing_round_id',null).eq('user_confirmed',false).neq('match_status','excluded').is('deleted_at',null);if(result.error)return;const count=(result.data??[]).filter((row:any)=>{if(row.proposed_fixture_id&&fixtureIds.has(row.proposed_fixture_id))return true;if(row.confirmed_fixture_id&&fixtureIds.has(row.confirmed_fixture_id))return true;const upload=Array.isArray(row.lead_testing_report_uploads)?row.lead_testing_report_uploads[0]:row.lead_testing_report_uploads;return Boolean(districtName)&&upload?.district_or_organization===districtName}).length;setUnresolved(count)})()},[fixtureIds,districtName,lead.rounds.length]);
