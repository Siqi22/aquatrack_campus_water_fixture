...
export function LeadReportUpload({onImported,reviewUnresolved=false}:{onImported?:()=>void|Promise<void>;reviewUnresolved?:boolean}){
  ...
  const hasReviewRows=rows.some(row=>!row.imported);
  ...
  return <div className="space-y-3"><div className="card-soft flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold">Review · {fileName}</p><p className="text-xs text-muted-foreground">{ready} included and ready · {needsReview} need review</p></div>{hasReviewRows&&<label className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-sm font-semibold ${allEligibleIncluded?'border-primary/40 bg-primary/5 text-primary':'border-border bg-background'} ${busy||bulkEligible.length===0?'cursor-not-allowed opacity-60':'cursor-pointer'}`}><Checkbox checked={allEligibleIncluded} disabled={busy||bulkEligible.length===0} onCheckedChange={checked=>void(checked===true?includeAllRows():excludeAllRows())}/><span>Include All ({bulkEligible.length})</span></label>}</div>{rows.map(row=><ReviewCard key={row.id} row={row} fixtures={fixtures} onCreate={()=>createFixtureForRow(row)} onChange={(patch,rematch)=>void changeRow(row,patch,rematch)}/>)}
