...
def _district_score(query: str, district: str) -> float:
    """Higher score = better match for autocomplete / fuzzy resolve."""
    q = query.strip().lower()
    d = district.lower()
    if not q:
        return 0.0
    if q == d:
        return 100.0
    if d.startswith(q):
        return 90.0 + (len(q) / max(len(d), 1)) * 5.0
    if q in d:
        return 70.0 + (len(q) / max(len(d), 1)) * 5.0
    return 0.0


def match_districts(query: str, limit: int = 10) -> list[str]:
    """
    Return district names matching query (case-insensitive substring + fuzzy fallback).
    ...
    scored = [(d, _district_score(q, d)) for d in districts]
    scored = [(d, s) for d, s in scored if s > 0]

    if not scored:
        fuzzy = difflib.get_close_matches(q, districts, n=limit, cutoff=0.45)
        return fuzzy
...
def resolve_district(query: str) -> str | None:
    """
    Resolve free-text input to a canonical district name, or None if no match.
    Prefers exact (case-insensitive) match, then best fuzzy match.
    """
    q = query.strip()
    if not q:
        return None
    ...
    matches = match_districts(q, limit=1)
