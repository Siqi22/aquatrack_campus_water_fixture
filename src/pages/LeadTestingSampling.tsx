...
export default function LeadTestingSampling(){const{campuses}=useFixtureStore();const{organizationName}=useOrganization();const districtName=resolveWorkspaceSchoolDistrict(campuses,organizationName);return <div className="page-shell max-w-4xl"><PageHeader title={`Record Lead Sampling Events for ${districtName}`} subtitle="Document lead sampling for fixtures"/><LeadTestingModuleNav/><BulkSampling/></div>}
