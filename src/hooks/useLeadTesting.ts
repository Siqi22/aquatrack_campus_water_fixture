...
export function useLeadTesting(fixtureId?: string) {
  ...
  async function startRemediation(item:Remediation){const now=new Date().toISOString();const {error}=await db.from('remediation_records').update({status:'in_progress',started_at:item.started_at||now}).eq('id',item.id);if(error)throw error;await db.from('fixtures').update({current_lead_testing_status:'remediation_in_progress',current_required_action:'Remediation required',lead_testing_last_updated_at:now}).eq('id',item.fixture_id);await event(item.fixture_id,'remediation_started',item.description||'Remediation started',{testing_round_id:item.triggering_testing_round_id,remediation_record_id:item.id});await refresh();}
  ...
  return {rounds,remediations,events,loading,refresh,createRound,enterResult,setAvailability,createRemediation,startRemediation,completeRemediation};
}
