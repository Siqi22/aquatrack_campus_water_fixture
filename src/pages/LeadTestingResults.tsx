...
function ResultGroup({title,items,tone='default'}:{title:string;items:ResultItem[];tone?:'default'|'warning'|'urgent'}){const ppbIndex=title.toLowerCase().indexOf('ppb');return <section><h2 className={`section-label mb-2 ${tone==='urgent'?'text-status-urgent':tone==='warning'?'text-status-warning':''}`}>{ppbIndex>=0?<>{title.slice(0,ppbIndex)}<span className="normal-case">ppb</span>{title.slice(ppbIndex+3)}</>:title} · {items.length}</h2><div className="overflow-x-auto rounded-xl border"><div className="min-w-[48rem]"><div className="grid grid-cols-[1fr_1fr_1.35fr_auto] gap-3 border-b bg-secondary/40 px-3 py-2 text-[10px] font-semibold text-muted-foreground"><span>School</span><span>Building</span><span>Fixture Location</span><span>Lead (ppb)</span></div>{items.map(item=><Link to={`/fixture/${item.round.fixture_id}`} key={item.round.id} className="grid grid-cols-[1fr_1fr_1.35fr_auto] gap-3 border-b px-3 py-3 text-xs last:border-b-0 hover:bg-secondary/30"><span>{item.school}</span><span>{item.building}</span><span>{item.location||'—'}</span><span className={`font-bold tabular-nums ${leadResultColor(item.round.result_ppb)}`}>{formatLeadMeasurement(item.round.result_value,item.round.result_original_unit,item.round.result_ppb).replace(/ ppb$/,'')}</span></Link>)}</div></div></section>}
