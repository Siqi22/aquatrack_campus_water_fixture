...
export const EXPORT_COLUMNS: ExportColumn[] = [
  ...
  { key: 'cleanliness', label: 'Cleanliness', get: (f) => f.qualityRating.cleanliness },
  { key: 'observations', label: 'Observations', get: (f) => f.observations ?? '' },
  { key: 'issues', label: 'Issues', get: (f) => (f.issues ?? []).join('; ') },
];

export const DEFAULT_EXPORT_KEYS: ExportColumnKey[] = [
  'id', 'campus', 'building', 'floor', 'room', 'brand', 'model',
...
export function exportToCSV(
  ...
  const rows = fixtures.map((f) =>
    cols.map((c) => escapeCSV(c.get(f, { campusName: campusNameById[f.campusId] }))),
  );
