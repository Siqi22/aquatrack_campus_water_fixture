...
import { CheckCheck, ChevronDown, FileCheck2, Link2, Plus, Search, Upload } from 'lucide-react';
...
export function LeadReportUpload({onImported,reviewUnresolved=false}:{onImported?:()=>void|Promise<void>;reviewUnresolved?:boolean}){
  ...
  return <div className="space-y-3"><div className="card-soft flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold">Review · {fileName}</p><p className="text-xs text-muted-foreground">{ready} matched and ready · {needsReview} need review</p></div>{confirmable.length>0&&<Button type="button" variant="outline" disabled={busy} onClick={()=>void confirmAllMatches()}><CheckCheck className="h-4 w-4"/>Confirm All ({confirmable.length})</Button>}</div>{rows.map(row=><ReviewCard key={row.id} row={row} fixtures={fixtures} onCreate={()=>createFixtureForRow(row)} onChange={(patch,rematch)=>void changeRow(row,patch,rematch)}/>)}
