...
interface ResultItem { round:TestingRound; school:string; building:string; location:string }
...
export default function LeadTestingResults(){
  ...
  const imported=useMemo<ResultItem[]>(()=>lead.rounds.filter(round=>fixtureIds.has(round.fixture_id)&&round.report_upload_id&&round.result_value).flatMap(round=>{const fixture=fixtures.find(item=>item.id===round.fixture_id);if(!fixture)return[];const campus=campuses.find(item=>item.id===fixture.campusId);if(!campus)return[];return[{round,school:campus.school,building:fixture.buildingName,location:[formatFloorLabel(fixture.floor),fixture.nearestRoom||fixture.roomNumber].filter(Boolean).join(' · ')}]}),[lead.rounds,fixtureIds,fixtures,campuses]);
  ...
  return <div className="page-shell"><PageHeader title="Lead Testing Results" subtitle="Imported laboratory results"/><LeadTestingModuleNav/>
...
function ResultGroup({title,items,tone='default'}:{title:string;items:ResultItem[];tone?:'default'|'warning'|'urgent'}){const ppbIndex=title.toLowerCase().indexOf('ppb');return <section><h2 className={`section-label mb-2 ${tone==='urgent'?'text-status-urgent':tone==='warning'?'text-status-warning':''}`}>{ppbIndex>=0?<>{title.slice(0,ppbIndex)}<span className="normal-case">ppb</span>{title.slice(ppbIndex+3)}</>:title} · {items.length}</h2><div className="overflow-x-auto rounded-xl border"><div className="min-w-[48rem]"><div className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1.35fr)_6rem] gap-3 border-b bg-secondary/40 px-3 py-2 text-left text-[10px] font-semibold text-muted-foreground"><span>School</span><span>Building</span><span>Fixture Location</span><span>Lead (ppb)</span></div>{items.map(item=><Link to={`/fixture/${item.round.fixture_id}`} key={item.round.id} className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1.35fr)_6rem] gap-3 border-b px-3 py-3 text-left text-xs last:border-b-0 hover:bg-secondary/30"><span className="min-w-0">{item.school}</span><span className="min-w-0">{item.building}</span><span className="min-w-0">{item.location||'—'}</span><span className={`whitespace-nowrap font-bold tabular-nums ${leadResultColor(item.round.result_ppb)}`}>{formatLeadMeasurement(item.round.result_value,item.round.result_original_unit,item.round.result_ppb).replace(/ ppb$/,'')}</span></Link>)}</div></div></section>}
